...
+ (u32)methodForSelector:(SEL)aSelector {
    // For the class-method variant, `this` IS the class object.
    // Cast it to Class and look up the IMP in the metaclass chain.
    let class = Class::from(this);
    match env.objc.get_imp(class, aSelector) {
        Some(imp) => {
            log_dbg!(
                "[+methodForSelector:] IMP={:#x} sel={:?} class={:?}",
                imp, aSelector, class
            );
            imp
        }
        None => {
            log!(
                "[+methodForSelector:] no IMP for sel={:?} on class {:?}, returning NULL",
                aSelector, class
            );
            0u32
        }
...
+ (u32)instanceMethodForSelector:(SEL)aSelector {
    // Look up in the *instance* method list of the class (not metaclass).
    let class = Class::from(this);
    match env.objc.get_imp(class, aSelector) {
        Some(imp) => imp,
        None => 0u32,
...
- (u32)methodForSelector:(SEL)aSelector {
    // Look up IMP on the instance's class (and superclasses).
    let isa: Class = ObjC::read_isa(this, &env.mem);
    match env.objc.get_imp(isa, aSelector) {
        Some(imp) => {
            log_dbg!(
                "[-methodForSelector:] IMP={:#x} sel={:?} class={:?}",
                imp, aSelector, isa
        ...
        None => {
            log!(
                "[-methodForSelector:] no IMP for sel={:?} on {:?}, returning NULL",
                aSelector, isa
            );
            0u32
        }
